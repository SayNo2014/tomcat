...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
            throws Exception {
    	// 1.将org.apache.coyote.Request和org.apache.coyote.Response对象
    	// 转变为org.apache.catalina.connector.Request,
    	// org.apache.catalina.connector.Response类型的对象
    	// coyote包中的Request仅仅只是包含了解析出来的http协议的数据
    	// connector包中的Request才是真正Servlet容器中的HttpServletRequest，
    	// 它里面包含了完成请求需要的host,context和wrapper信息
        Request request = (Request) req.getNote(ADAPTER_NOTES);
        Response response = (Response) res.getNote(ADAPTER_NOTES);

        if (request == null) {
            // 构造servlet的request对象和response对象
        ...
        try {
            ...
            if (postParseSuccess) {
                ...
                // container容器的管道链进行处理（Service是Container的最顶层）
                connector.getService().getContainer().getPipeline().getFirst().invoke(
                        request, response);
            }
            if (request.isAsync()) {
