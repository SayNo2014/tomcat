...
public class Catalina {
    ...
    public void setAwait(boolean b) {
        // 设置await参数为true，允许Server启动后进入等待状态，即线程阻塞
    	await = b;
    }
    ...
    public void load() {
        ...
        // 初始化Server
        try {
        	// 实际调用org.apache.catalina.util.LifecycleBase的init方法
            getServer().init();
        } catch (LifecycleException e) {
    ...
    public void start() {
        ...
        // 这个方法是在jvm中增加一个关闭的钩子，当jvm关闭的时候，会执行系统中已经设置的所有通过方法addShutdownHook添加的钩子
        // 当系统执行完这些钩子后，jvm才会关闭。
        // 这个钩子可以在以下几种场景被调用:
        // 1.程序正常退出
        // 2.使用System.exit()
        // 3.终端使用Ctrl+C(linux系统中)触发的中断
        // 4.系统关闭
        // 5.使用kill pid命令干掉进程(Kill -9 不会触发钩子)
        if (useShutdownHook) {
