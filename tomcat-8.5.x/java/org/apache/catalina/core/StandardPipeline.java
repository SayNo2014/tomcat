...
public class StandardPipeline extends LifecycleBase
    ...
    public void addValve(Valve valve) {
        ...
        // 如果first为空时，将value设置为first，并设置basic为Next
        // 如果first不为空代表Value数组中有值，那么临时变量current保存first值，
        if (first == null) {
            first = valve;
            valve.setNext(basic);
        } else {
            Valve current = first;
            while (current != null) {
            	// 如果管道中只有一个value和basic直接设置原来first的next为新添加的value
            	// 设置新添加的value的next指向basic
            	// 如果管道中有多个value和basic，调用first value的getNext一直到最后一个
            	// 普通value，然后将传递的参数value设置到最后一个普通value的后面，并且设置
            	// 参数value的next为basic
            	// 其实就是通过这个算法将传递的传输加载到普通value的最后一个
