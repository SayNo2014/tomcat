...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // StartChild 实现了 Callable，实现了Callable接口的类可以防止到线程池内执行
        // 线程池会默认调用StartChild的call方法
    ...
    protected void threadStart() {
        ...
        if (thread != null)
            return;
        // backgroundProcessorDelay默认值为-1，以下逻辑不会被执行
        // StandardEngine中设置backgroundProcessorDelay为10
        if (backgroundProcessorDelay <= 0)
            return;

        threadDone = false;
        // StandardEngine启动时：ContainerBackgroundProcessor[StandardEngine[Catalina]]
